...
- `apps/brain_cloud` owns Ecto/PostgreSQL, users, organizations, owner-managed memberships, scoped token digests, soft-deactivated teams and retained membership links, direct/team reader-editor project grants with strongest-access resolution, implicit owner access, final-owner/team-row locking, transactional credential revocation, immutable audit events, organization-owned projects, immutable memory revisions, exact SHA-256 content hashes, tenant/project-scoped `simple` full-text search, system information, readiness, release migrations/bootstrap, and domain supervision.
...
- Implemented protected routes are token create/list/revoke, membership create/list/role/deactivate/reactivate/target-token, team lifecycle/membership, project creation, direct/team project access list/put/delete, memory creation/retrieval, and keyword search. Root, health, readiness, and system discovery remain public.
