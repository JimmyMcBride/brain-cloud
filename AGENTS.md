...
Current Phase 2H implementation is a Phoenix umbrella: `apps/brain_cloud` owns Ecto/PostgreSQL, accounts, organization tenancy, memberships/invitations, human/agent API token digests, browser login challenges and tracked sessions, global auth events, teams/agents and retained links/grants, project access, audit provenance, projects, memories, search, release bootstrap/migrations, and domain supervision; `apps/brain_cloud_web` owns Phoenix, Bandit, separate API bearer and human browser authentication, synchronous Swoosh sign-in delivery, encrypted cookies, database-rehydrated LiveView scope, public invitation acceptance, API authorization, and the minimal identity shell. There is no worker application. Never imply product CRUD UI or later roadmap features exist. Run `make check`, `make upgrade-phase2`, and runtime `make smoke-phase2` after server changes. Automatic invitation delivery, interactive invitation acceptance, owner invitations, alternate authenticators, agent proposals, agent project creation/administration, custom roles, nested teams, deny rules, and a generic policy engine remain later work.
