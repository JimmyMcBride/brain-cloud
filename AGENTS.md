...
Current Phase 2C implementation is a Phoenix umbrella: `apps/brain_cloud` owns Ecto/PostgreSQL, accounts, organization tenancy, owner-managed human memberships, scoped token digests, direct reader/editor project grants, implicit owner access, final-owner locking, transactional credential revocation, immutable audit events, organization-owned projects, immutable memory revisions, tenant/project-scoped keyword search, release bootstrap/migrations, and domain supervision; `apps/brain_cloud_web` owns Phoenix, Bandit, persisted bearer authentication, owner/fixed-scope/project-access enforcement, LiveView, and HTTP transport. There is no separate worker application until jobs exist. Never imply later roadmap features already work. Run `make check`, `make upgrade-phase2`, and runtime `make smoke-phase2` after server changes. Invitations, teams, interactive login, agent credentials, custom roles, group grants, and a generic policy engine remain later work.
