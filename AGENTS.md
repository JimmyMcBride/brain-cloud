...
Current Phase 2D implementation is a Phoenix umbrella: `apps/brain_cloud` owns Ecto/PostgreSQL, accounts, organization tenancy, owner-managed human memberships, scoped token digests, soft-deactivated teams and retained membership links, direct/team reader-editor project grants with strongest-access resolution, implicit owner access, final-owner/team-mutation locking, transactional credential revocation, immutable audit events, organization-owned projects, immutable memory revisions, tenant/project-scoped keyword search, release bootstrap/migrations, and domain supervision; `apps/brain_cloud_web` owns Phoenix, Bandit, persisted bearer authentication, owner/fixed-scope/project-access enforcement, LiveView, and HTTP transport. There is no separate worker application until jobs exist. Never imply later roadmap features already work. Run `make check`, `make upgrade-phase2`, and runtime `make smoke-phase2` after server changes. Invitations, interactive login, agent credentials, custom roles, nested teams, deny rules, and a generic policy engine remain later work.
