...
Use `make check` for formatting, warnings-as-errors compilation, database migrations/tests, and production assets. Use `make upgrade-phase2` with PostgreSQL running to verify the Phase 1 data upgrade/adoption path plus selective Phase 2D/2E scope migrations with no team or agent backfill. Against a running Compose project, use `make smoke-phase2` to verify release bootstrap/recovery, human and agent token lifecycle, direct/team/agent project access, dormant lifecycle behavior, two-organization isolation, create/retrieve/search, API restart durability, PostgreSQL outage readiness, and recovery. Active integration work targets `develop`; release stabilization uses `release/vX.Y.Z`; `main` contains releases. Plan owns GitHub planning artifacts whenever source mode is `github`.
