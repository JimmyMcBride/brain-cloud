...
Phase 2G is a Phoenix umbrella with one identity-, tenant-, and project-protected persistent vertical slice. `apps/brain_cloud` owns Ecto/PostgreSQL, users, organizations, owner-managed memberships and member invitations, human and agent token digests plus one-time invitation-secret digests, soft-deactivated teams and agents, retained links/grants, direct human/team reader-editor grants, direct agent reader-editor grants, strongest-access resolution for humans, implicit owner access, row-locked lifecycle mutations, transactional credential revocation, immutable human-or-agent audit events, organization-owned projects, immutable human-or-agent memory revisions, synchronous organization/project-scoped `simple` full-text search, compatibility metadata, readiness, release migrations/bootstrap, and domain supervision. PostgreSQL composite foreign keys enforce tenant alignment across principals, projects, and grants. `apps/brain_cloud_web` owns Phoenix, Bandit, persisted bearer authentication with explicit human/agent provenance, owner/scope plugs, project authorization before memory/search lookup, JSON controllers, LiveView, and the minimal web shell.

The server exposes public one-time invitation acceptance plus human and agent credential lifecycle, owner-managed memberships and member invitations, teams and agents, direct/team/agent project grants, project creation, memory creation/retrieval, and keyword search under `/v1`. Authentication derives an explicit human or agent principal plus organization provenance. Ecto queries enforce tenant and project authorization before protected content retrieval. Owners are implicit; human readers/editors retain their existing access, while agents require direct reader or editor access plus independent read, search, or write scopes. Exact content hashes and immutable revision 1 remain wire compatible with Phase 1. OTP handles graceful supervision; no separate worker application exists until real jobs require one. No invitation delivery, interactive identity, owner invitations, proposals, custom-role ACLs, module registry, Planning domain, arbitrary loading, or external process protocol exists yet.
