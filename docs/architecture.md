...
Phase 2H is a Phoenix umbrella with one identity-, tenant-, and project-protected persistent vertical slice. `apps/brain_cloud` owns Ecto/PostgreSQL, users, organizations, owner-managed memberships and member invitations, human and agent API token digests, one-time invitation and browser-login digests, tracked browser sessions, global human-auth security events, soft-deactivated teams and agents, retained links/grants, direct human/team/agent reader-editor grants, row-locked lifecycle mutations, transactional credential revocation, organization audit, projects, immutable human-or-agent memory revisions, keyword search, readiness, release migrations/bootstrap, and domain supervision. PostgreSQL composite foreign keys enforce tenant and selected-membership alignment. `apps/brain_cloud_web` owns Phoenix, Bandit, API bearer authentication, closed-enrollment passwordless sign-in, synchronous Swoosh delivery, encrypted browser cookies, database-rehydrated LiveView scope, JSON controllers, and the minimal identity shell.

The `/v1` API contract is unchanged: human and agent bearer credentials remain separate from browser sessions. Browser scope reloads the user, active memberships, selection, organization, and role from PostgreSQL on each HTTP request and LiveView mount/reconnect. One active membership selects automatically; multiple memberships require server-validated selection. Challenge consumption and seven-day session reissue use row locks, sessions expire absolutely after 14 days, and membership deactivation is visible on the next request. OTP handles graceful supervision; synchronous mail adds no worker. Automatic invitation delivery, interactive invitation acceptance, owner invitations, proposals, product CRUD UI, custom-role ACLs, module registry, Planning domain, arbitrary loading, and external process protocol do not exist yet.
