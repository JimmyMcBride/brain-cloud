...
Phase 2D is a Phoenix umbrella with one identity-, tenant-, and project-protected persistent vertical slice. `apps/brain_cloud` owns Ecto/PostgreSQL, users, organizations, owner-managed memberships, scoped token digests, soft-deactivated teams and retained membership links, direct and team reader/editor project grants, strongest-access resolution, implicit owner access, final-owner and team-row locking, transactional credential revocation, immutable audit events, organization-owned projects, immutable memory revisions, synchronous organization/project-scoped `simple` full-text search, compatibility metadata, readiness, release migrations/bootstrap, and domain supervision. PostgreSQL composite foreign keys enforce project/membership/team/grant tenant alignment. `apps/brain_cloud_web` owns Phoenix, Bandit, persisted bearer authentication, owner/scope plugs, project authorization before memory/search lookup, JSON controllers, LiveView, and the minimal web shell.

The server exposes token create/list/revoke, owner-managed membership lifecycle and target credentials, project access list/grant/change/revoke, project creation, memory creation/retrieval, and keyword search under `/v1`. Authentication derives user and organization provenance; Ecto queries enforce tenant and direct project access before protected content retrieval. Owner access is implicit. Readers can retrieve/search; editors can also create memories. Exact content hashes and immutable revision 1 remain wire compatible with Phase 1. OTP handles graceful supervision; no separate worker application exists until real jobs require one. No invitations, interactive identity, teams, group/custom-role ACLs, module registry, Planning domain, arbitrary loading, or external process protocol exists yet.
